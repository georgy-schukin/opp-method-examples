...
int main(int argc, char **argv) {
    ...
    #pragma omp parallel for schedule(dynamic, SIZE) collapse(2)
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
    ...
    do {
        ...
        #pragma omp for reduction(max:norm) schedule(dynamic, SIZE) collapse(2)
        for (int i = 1; i < N - 1; i++) {
            for (int j = 1; j < N - 1; j++) {
        ...
        #pragma omp for schedule(dynamic, SIZE) collapse(2)
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
